    ...
    class Indexer < Jekyll::Generator
      def initialize(config = {})
        ...
        @lunr_config = {
          ...
          'js_dir' => 'js', 
          'ctf_version' => '/teamforge212' #You must change this variable before building your site for every release
        }.merge!(config['lunr_search'] || {})

        @js_dir = @lunr_config['js_dir']
        @ctf_version = @lunr_config['ctf_version']
        gem_lunr = File.join(File.dirname(__FILE__), "./lunr.js")
      ...
      def pages_to_index(site)
        ...
        items.reject! {|i| i.data['exclude_from_search'] || i.data['search'] == 'exclude' }
...
module Jekyll
  module LunrJsSearch
    class SearchEntry
      ...
      attr_reader :title, :url, :date, :keywords, :tags, :is_post, :body, :collection

      def initialize(title, url, date, keywords, tags, is_post, body, collection)
        @title, @url, @date, @keywords, @tags, @is_post, @body, @collection = title, url, date, keywords, tags, is_post, body, collection
      end
